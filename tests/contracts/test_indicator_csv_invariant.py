...
def _find_indicator_csvs() -> list[Path]:
    return sorted(LOGS_DIR.glob(f"{TICKER}_*_{TAG}_indicators.csv"))


@pytest.mark.parametrize("csv_path", _find_indicator_csvs(), ids=lambda p: p.stem)
def test_indicator_csv_header_starts_with_timestamp(csv_path: Path):
...
@pytest.mark.parametrize("csv_path", _find_indicator_csvs(), ids=lambda p: p.stem)
def test_indicator_csv_has_enriched_data(csv_path: Path):
...
@pytest.mark.parametrize("csv_path", _find_indicator_csvs(), ids=lambda p: p.stem)
def test_indicator_csv_timestamp_parseable(csv_path: Path):
