...
def test_timeout_late_worker_requires_fresh_b_and_never_publishes_a(monkeypatch):
    ...
    manager = OrderManager(
        ...
        broker_adapter=SimpleNamespace(
            place_order=lambda *a, **k: sends.append((a, k))),
        execution_context=mon._execution_context,
    )
    mon.order_mgr = manager
    counters = {"reconcile": 0, "apply_fill": 0}
    mon._reconcile_local_orders_from_snapshot = lambda *_a, **_k: counters.__setitem__(
        "reconcile", counters["reconcile"] + 1)
    mon._apply_confirmed_futures_deal = lambda *_a, **_k: counters.__setitem__(
        "apply_fill", counters["apply_fill"] + 1)
    ...
    assert mon._broker_refresh_channel_state == "UNCERTAIN"
    assert manager._broker_refresh_channel_state == "UNCERTAIN"
    assert counters == {"reconcile": 0, "apply_fill": 0}

    order = manager.create_order("TMFI6", OrderSide.BUY, OrderType.MARKET, 1)
    ...
    assert mon._broker_refresh_channel_state == "HEALTHY"
    assert manager._broker_refresh_channel_state == "HEALTHY"
    assert [kind for kind, _ in api.calls] == ["update", "list", "update", "list"]
    assert mon._broker_refresh_channel_history[-1] == "HEALTHY"
