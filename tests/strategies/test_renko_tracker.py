# 2026-08-02 Antigravity: Updated unit tests for RenkoTracker (3-tuple return)
import pytest
...
def test_renko_initial_bricks_upward():
    tracker = RenkoTracker(anchor_price=44000.0, brick_size=10.0)
    num_bricks, trend, _ = tracker.add(44012.0)
    assert num_bricks == 1
    assert trend == 1
    assert tracker.renko_open == 44000.0
...
def test_renko_noise_filtering():
    ...
    num_bricks, trend, _ = tracker.add(44018.0)
    assert num_bricks == 0
    assert tracker.renko_close == 44010.0
    
    num_bricks, trend, _ = tracker.add(44002.0)  # Drop to 44002 (above reversal threshold 43990)
    assert num_bricks == 0
    assert tracker.renko_close == 44010.0
...
def test_renko_bullish_continuation():
    ...
    num_bricks, trend, _ = tracker.add(44032.0)
    assert num_bricks == 2
    assert trend == 1
...
def test_renko_2brick_reversal():
    ...
    num_bricks, trend, _ = tracker.add(43989.0)
...
def test_renko_dynamic_atr_update():
    tracker = RenkoTracker(anchor_price=44000.0, brick_size=10.0)
    # Item A: brick_size is locked per episode
...
def test_renko_zero_or_none_safety():
    tracker = RenkoTracker(anchor_price=44000.0, brick_size=10.0)
    num_bricks, trend, _ = tracker.add(0.0)
    assert num_bricks == 0
    num_bricks, trend, _ = tracker.add(None)
    assert num_bricks == 0
