...
def cancel_stale_order():
    ...
    # 訂單文件路徑
    orders_file = Path("exports/trades/TMF_20260415_orders.json")
    
    if not orders_file.exists():
        print(f"❌ 訂單文件不存在: {orders_file}")
        return False
    ...
    target_order = None
    for order in orders:
        if order['order_id'] == 'ORD-000002':
            target_order = order
            break
    
    if not target_order:
        print("❌ 找不到ORD-000002訂單")
        return False
    ...
    # 檢查價格差距
    current_price = 37034  # 從dashboard獲取
    if 'price' in target_order and target_order['price']:
        price_diff = abs(current_price - target_order['price'])
        price_diff_pct = price_diff / current_price * 100
        print(f"💰 價格差距: {price_diff:.1f}點 ({price_diff_pct:.2f}%)")
...
def check_order_lifecycle_issues():
    ...
    if pending_orders:
        print(f"⚠️  發現 {len(pending_orders)} 個待處理訂單:")
        for order in pending_orders:
            created_time = datetime.fromisoformat(order['created_at'].replace('Z', '+00:00'))
            age_hours = (datetime.now() - created_time).total_seconds() / 3600
            print(f"  - {order['order_id']}: {order['side']} {order['quantity']}口 @ {order.get('price', '市價')} (已等待{age_hours:.1f}小時)")
    else:
        print("✅ 沒有待處理訂單")
...
def main():
    ...
    # 檢查訂單生命週期問題
    check_order_lifecycle_issues()
    
    print("\n" + "="*50)
    
    # 取消過時訂單
    success = cancel_stale_order()
