# 唯讀歷史單腿 Policy J 歸因稽核 — 設計文件 v5

**狀態**: DESIGN v5（codex 審查中；通過後才實作 committed script + artifact）
...
已知型別與其 allowed side enum（實測對映）：
  ...
  TEST                 → side ∈ {BUY, SELL, ...}（已知非候選/測試污染，排除+計數）
...
candidate 成立（同一 trade_id，全部必要）：
  ...
  5. 無 COMBINED_EXIT* 同 trade（排除型別；含之 → 拒絕）
  6. TEST 型別行不參與候選建構（計入 excluded_trades 或 rejected 清單）
  7. 拒絕條件（計入 rejected_candidates + reason）：partial（qty<=0 或非整數）、
     multi-event、未知 fill_type（→ 快照層，§6）、side 缺失、price<=0、COMBINED_EXIT 混入
```
...
- **時間解析（v5）**：一律 `datetime.fromisoformat`（timezone-aware 優先）：
  - naive（無 offset）→ 依 `parser_assumptions["naive_ts_mean"]` 視為台灣本地（UTC+8），
    該候選標 `source_limit=TS_NAIVE`
  - mixed offset / 無法確立語意 → 相關候選 NOT_PROVABLE（`TS_SEMANTICS_UNKNOWN`）；
    若 parser 層完全無法確立（如全檔無 offset 且無本地時區聲明可依）→ 整份 UNREADABLE
  - missing timestamp → 該候選 NOT_PROVABLE（`TS_MISSING`）
...
3. **enum 驗證（由快照觀察 + allowlist 對照）**：
   ...
   - 與 §2 allowlist 對照：fill_type/side 在 allowlist 內 → 正常；`TEST` → 已知非候選型別，
     排除於候選 + 計數回報（`test_rows`），**不 UNREADABLE**；allowlist 外值 →
     `status=UNREADABLE` + 該值細節
   - COMBINED_EXIT* 型別以**排除型別**處理（含其 trade 拒絕為候選），allowed side 為
     {BUY,SELL,NONE,""}（實測），不做 settlement-only 限制
    ...
    "inputs": {
      "fills":  {"sha256": "...", "bytes": n, "snapshot_read_ts": iso,
                 "source_schema": {
                   ...
                   "observed_enum_by_fill_type": {"ENTRY": ["LONG","SHORT"], "EXIT": ["BUY","SELL"],
                                                  "COMBINED_EXIT": ["BUY","SELL","NONE",""], "TEST": ["SELL"], ...},
                   "allowlist_ok": true,
                   "test_rows": n,
      ...
      "events": {"sha256": "...", "bytes": n, "snapshot_read_ts": iso,
                 "source_schema": {
                   ...
                   "per_trade_evidence_types_missing_trade_id": {"PEAK_CONFIRMED": n, ...},
                   "timestamp_offsets": {"+08:00": n, "naive": n, "other": n}}}
    },
