...
## 摩擦成本分析 (Friction Costs)

### ⚠️ 重要: 所有 PnL 必須顯示淨損益 (扣除成本後)

RULES.md Rule 4: "PnL Must Include All Costs — NEVER show gross PnL without deducting costs"
...
| 券商佣金 | 20 × 2邊 × 1口 | 40 |
| 交易所費 | 0 × 2邊 × 1口 | 0 |
| 交易稅 | (進+出) × 50 × 0.00002 × 1口 | ~13 (33000 pts) |
| 滑價 | 1 pts × 50 × 1口 | 50 |
| **總成本/口** | | **~103 TWD** |
| **總成本/2口** | | **~206 TWD** |
| **損益兩平點** | | **+3~4 pts** |
...
| 券商佣金 | 20 × 2邊 × 1口 | 40 |
| 交易所費 | 5 × 2邊 × 1口 | 10 |
| 交易稅 | (進+出) × 50 × 0.001 × 1口 | ~10 (權利金100) |
| 滑價 | Bid-Ask spread (~1-2 pts) | 50-100 |
| **總成本/口** | | **~110-160 TWD** |

### 摩擦成本對策略的影響

| 策略 | 平均獲利/筆 | 摩擦成本/筆 | 淨獲利 | 影響 |
|------|------------|------------|--------|------|
| Counter-VWAP | ~376 TWD | ~206 TWD | ~170 TWD | 成本佔 55% |
| PSAR Breakout | ~275 TWD | ~206 TWD | ~69 TWD | 成本佔 75% |
| 選擇權 TP1 | ~600 TWD | ~160 TWD | ~440 TWD | 成本佔 27% |

### 實戰檢查清單

每次出場前確認：
- [ ] 毛利 > 摩擦成本 (至少 2x 成本才值得出場)
- [ ] 淨損益顯示在 Dashboard 和 CSV 中
- [ ] 停損點已包含摩擦成本 (不能只看毛利)
