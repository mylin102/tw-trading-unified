...
class AttributionRecorder:
    """
    In-memory recorder for router attribution.
    ...
    def export_csv(self, output_dir: str | Path) -> None:
        ...
        self.router_df().to_csv(output / "router_evaluation_log.csv", index=False)
        self.signal_df().to_csv(output / "strategy_signal_log.csv", index=False)
        self.trade_df().to_csv(output / "trade_attribution_log.csv", index=False)
...
def summarize_router(router_df: pd.DataFrame) -> pd.DataFrame:
    ...
    candidate_count is approximated as all rows per strategy in router_df.
    """
    if router_df.empty:
        return pd.DataFrame()
