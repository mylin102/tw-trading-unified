...
class ExecutionContext:
    ...
    def assert_order_allowed(self, order, *, method: str) -> None:
        ...
        if self.is_live_ready():
            return
        # [QUARANTINE_EXIT 2026-08-18] LIVE_QUARANTINED narrow carve-out:
        # ONLY snapshot-bound remaining-leg MTS_RELEASE / MTS_EXIT orders
        # may submit; the per-order proof is generated by the monitor
        # AFTER a fresh broker-truth recheck.  Entry / manual / OCO /
        # generic cancel stay default-deny; effective_mode itself is
        # never changed here.
