...
def main():
    ...
    if args.json:
        ...
        overall = _fmt(summary.ratings.overall_rating)
        gate_blocked = (overall == "CAPTURE_INVALID")

        print(f"Overall Rating:              {overall}")
        print(f"Execution Gate Blocked:      {gate_blocked}")
        print("---------------------------------------------------------")
        print("Subsystem Ratings:")
        print(f"  - Monotonicity:            {_fmt(summary.ratings.ordering_rating)}")
        print(f"  - Duplicates:              {_fmt(summary.ratings.integrity_rating)}")
        print(f"  - Freshness:               {_fmt(summary.ratings.freshness_rating)}")
        print(f"  - Writer Health:           {_fmt(summary.ratings.writer_rating)}")
        print(f"  - Boundary Contamination:  {_fmt(summary.ratings.generation_rating)}")
        print("---------------------------------------------------------")
