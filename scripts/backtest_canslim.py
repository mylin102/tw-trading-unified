#!/usr/bin/env python3
"""
CANSLIM Backtest — Taiwan Stock Market (Technical Proxy Version)
...
def canslim_score(ticker, df, date, market_ret=0):
    ...
    total = c_score + a_score + n_score + s_score + l_score
...
class Position:
    ...
    def check_exit(self, price):
        if price <= self.entry_price * (1 - STOP_LOSS):
            return 'STOP_LOSS'
...
class Backtester:
    ...
    def _buy(self, ticker, price, date, reason=""):
            ...
            if d in rebal_dates:
                ...
                for t in selected:
                    if t not in self.positions:
                        ...
                        mask = df.index <= date
                        if mask.sum() > 0:
                            self._buy(t, df[mask].iloc[-1]['Close'], date, f"CANSLIM")
