...
def run(label, bull, bear):
    ...
    for i in range(1, n):
        c, s, h, lo, v = close[i], score_arr[i], high[i], low[i], vwap[i]

        if pos != 0:
            # TP1
            if not has_tp1 and lots == LOTS:
                tp_hit = (h >= entry_p + TP1_PTS) if direction == 1 else (lo <= entry_p - TP1_PTS)
                if tp_hit:
        ...
        if pos == 0:
            ...
            if not pd.isna(df_15m_a.iloc[i]["ema_filter"]):
                can_long = df_15m_a.iloc[i]["Close"] > df_15m_a.iloc[i]["ema_filter"] * 0.998
                can_short = df_15m_a.iloc[i]["Close"] < df_15m_a.iloc[i]["ema_filter"] * 1.002
            else:
                can_long = True
                can_short = True
