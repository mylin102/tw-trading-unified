...
with st.sidebar:
    ...
    try:
        res = subprocess.run(["pgrep", "-f", "ui/backtest_dashboard.py"], capture_output=True, text=True)
        if res.returncode == 0 and res.stdout.strip():
            is_8502_running = True
    except Exception:
        pass

    if is_8502_running:
        st.success("🟢 8502 實驗室執行中")
        st.markdown('<a href="http://localhost:8502" target="_blank" style="text-decoration:none;"><button style="width:100%;padding:8px;background-color:#2e7d32;color:white;border:none;border-radius:4px;cursor:pointer;font-weight:bold;margin-bottom:6px;">🚀 開啟 8502 實驗室</button></a>', unsafe_allow_html=True)
        if st.button("⏹️ 關閉 8502 實驗室 (釋放資源)", use_container_width=True):
            try:
                subprocess.run(["pkill", "-f", "ui/backtest_dashboard.py"])
                st.info("🛑 8502 實驗室已關閉並釋放資源！")
                st.rerun()
...
def build_exit_only_attestation_request(
        ...
        expected_legs=None):
    """Build the monitor's unchanged ATTEST_EXIT_ONLY command schema.

    When a current capability is supplied, its two legs are the sole source
    of the request scope.  The optional ``expected_legs`` exists only for
    the initial, no-capability attestation form and is normalized to the same
    strict schema.
    """
...
def write_exit_only_attestation_request(path, payload):
    ...
    ``False`` means a pending request already exists.  Other OS failures are
    allowed to reach the caller so the dashboard can state that no command
    was accepted.
    """
    path = Path(path)
        ...
        if _mts_enabled:
            ...
            with st.expander("🛡️ 對帳部位：受限平倉授權", expanded=False):
                ...
                if _pending_attestation:
                    ...
                    _attestation_capability = None
                    _attestation_legs = None
                    ...
                    if _submit_attestation:
                        try:
                            _payload = build_exit_only_attestation_request(
                                _attestation_capability,
                                operator=_operator,
                                trade_id=_trade_id,
                                evidence=_evidence,
                                expected_legs=_attestation_legs)
                            if write_exit_only_attestation_request(_attest_path, _payload):
                                st.success(
                                    "✅ 授權請求已寫入；等待 Monitor 以新鮮券商快照驗證。")
                            else:
                                st.warning("已有待處理授權請求，未覆蓋。")
                        except ValueError as exc:
                            st.error(f"無法建立受限平倉授權請求：{exc}")
                        except OSError:
                ...
                with st.expander(f"📝 已完結交易清單 (Closed Loops - {_sel_session})", expanded=True):
                    ...
                    for t in _completed_filtered:
                        ...
                        _loop_rows.append({
                            ...
                            "近月 PnL": f'{t.get("near_pnl_gross", t.get("release_pnl", 0)):+,.0f}',
                            "近月原因": ("— (雙腿同步平倉)" if _is_pj else _fmt_policy_j_reason(t.get("release_reason", "—"))),
                            "近月來源": f'{t.get("release_reason_source", "—")}',
                            "遠月 PnL": f'{t.get("far_pnl_gross", t.get("exit_pnl", 0)):+,.0f}',
                            "遠月原因": _fmt_policy_j_reason(t.get("exit_reason", "—")),
                            "遠月來源": f'{t.get("exit_reason_source", "—")}',
